...
# Brian ensemble standard-deviation bands
PLOT_BRIAN_STD = True
...
# Domain
# IHere v_min, v_max are interpreted in volt/s
...
def sample_truncated_normal(rng, mean, sigma, low, high, size):
    """
    Sample from N(mean, sigma^2) conditioned to lie in [low, high].
    For the present parameters, about 95% of x-samples and almost all v-samples
    are accepted, so rejection sampling is cheap.
    """
    ...
    while filled < size:
        ...
        # Oversample to keep the number of rejection loops small.
...
for k in range(1, Nt):
    ...
    if (k % 1000 == 0) or (k == Nt - 1):
        print(
            ...
            f"N_k_minus_1={N_k_minus_1:.6e}, mass={current_mass:.16f}, mean={current_mean_x/U0_volt:.16f}"
        )
...
if RUN_BRIAN:
    ...
    try:
        ...
        for run_id in range(N_RUNS):
            ...
            # Continuous Brian initial condition corresponding to the Gaussian
            # used to define the PDE initial datum, conditioned to lie inside
            # the domain. 
        ...
        if N_RUNS > 1:
            std_activity_brian = np.std(activity_runs, axis=0, ddof=1)
            std_mean_x_brian_interp = np.std(mean_x_runs, axis=0, ddof=1)
...
if RUN_BRIAN and (mean_activity_brian is not None):
    line_brian, = ax1.plot(tb_bin, mean_activity_brian, '--', lw=1,
                           label='Brian 2 mean')

    if PLOT_BRIAN_STD and (std_activity_brian is not None) and (N_RUNS > 1):
        ax1.fill_between(
            tb_bin,
...
if RUN_BRIAN and (mean_x_brian_interp is not None):
    ...
    line_brian_x, = ax2.plot(brian_times_mean[:Lm], mean_x_brian_interp[:Lm],
                             '--', lw=1.0, label='Brian 2 mean')

    if PLOT_BRIAN_STD and (std_mean_x_brian_interp is not None) and (N_RUNS > 1):
        ax2.fill_between(
            brian_times_mean[:Lm],
...
if RUN_BRIAN and (rho_x_brian_final is not None):
    line_brian_rho, = ax3.plot(x_hist_centers, rho_x_brian_final, '--', lw=1.0,
                               label='Brian 2 mean')

    if PLOT_BRIAN_STD and (std_rho_x_brian_final is not None) and (N_RUNS > 1):
        ax3.fill_between(
            x_hist_centers,
...
if RUN_BRIAN and (mean_activity_brian is not None):
    ...
    if N_RUNS > 1:
        print("Brian standard-deviation bands saved on the plots.")
print("Saved graphs:")
print(" - activity")
print(" - mean voltage")
print(" - final density")
