# -*- coding: utf-8 -*-
"""
The script performs an h-sweep at fixed Delta t.

Outputs:
- for each value of h: comparison plots for N(t), X(t), rho(x,T);
- a CSV table of L2 distances against the Brian ensemble mean;
- ordinary plots of h versus L2 distance.

The relative values correspond to divisions by the norm of the
corresponding Brian 2 observable.
...
# If True, Brian2 is run once for the whole h-sweep, using the fixed
...
#
# If False, Brian2 is run separately for each value of h.
...
# the PDE grids. We use the same value as the fixed PDE time step.
BRIAN_REFERENCE_DT = DT_FIXED
...
# Associated grid level
n = int(round(1.0 / h - 1.0))

# PDE time steps
Nt_user = int(round(T / DT_FIXED)) + 1
...
def plot_summary(records, outdir):
    ...
    h_values = sorted(set(float(get_record_value(row, 'h')) for row in records))

    def value_at(h_value, key):
        for row in records:
            if float(get_record_value(row, 'h')) == float(h_value):
                return float(get_record_value(row, key))
        return float('nan')

    for obs_name, key, ylabel in observables:
        yvals = np.array([value_at(h_value, key) for h_value in h_values])

        fig, ax = plt.subplots(figsize=(7, 4))
        ax.plot(h_values, yvals, marker='o', lw=1.0)
        ax.set_xlabel(r"$h$")
        set_horizontal_ylabel(ax, ylabel)
        ax.set_title(rf"Variation with $h$: {obs_name}, $\Delta t={DT_FIXED:.0e}$")
        fig.tight_layout()
        fig.subplots_adjust(left=0.30)

        if SAVE_PDF:
            fig.savefig(
                os.path.join(outdir, f"summary_h_L2_{obs_name}.pdf"),
                bbox_inches='tight',
            )
        if SAVE_PNG:
            fig.savefig(
                os.path.join(outdir, f"summary_h_L2_{obs_name}.png"),
                dpi=DIAG_PLOT_DPI,
                bbox_inches='tight',
            )
        plt.close(fig)
...
def main():
    ...
    for h_value in H_VALUES:
        ...
        stem = (
            f"compare_h{fmt_float_for_filename(h)}_"
            f"dt{fmt_float_for_filename(delta_t)}_"
            f"Nx{Nx}_Nv{Nv}_Nt{Nt_user}"
        )
    ...
    print(" - ordinary h versus L2 distance")
