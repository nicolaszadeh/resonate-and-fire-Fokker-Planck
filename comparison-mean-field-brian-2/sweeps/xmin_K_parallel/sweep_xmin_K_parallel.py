# -*- coding: utf-8 -*-
"""
The parameters x_min and K are swept in parallel.

Outputs:
- for each pair (x_min,K): comparison plots for N(t), X(t), rho(x,T);
- a CSV table of L2 distances against the Brian ensemble mean;
- summary plots for one fixed value in the pair while the other one varies.

The relative values correspond to divisions by the norm of the corresponding
Brian 2 observable.

Nicolas Zadeh, June the 15th, 2026
"""
...
# If True, Brian2 is run once per K, using the largest left domain as the
...
#
# If False, Brian2 is run separately for each pair (x_min,K).
REUSE_BRIAN_ACROSS_XMIN = True
...
# Sweep values.
XMIN_VALUES = [-21.0, -26.0, -31.0, -36.0]
K_VALUES = [1.0e-3, 1.0e-4, 1.0e-5, 1.0e-6]
...
def update_kick_dependent_parameters(K_value):
    global K, J, J_E_ext, J_I_ext, J_E_int, J_I_int
    ...
    # The actual velocity kick size is K = J/(C*tau_syn), hence
    # J = K*C*tau_syn.
    J = K * C_phys * tau_syn_phys
...
# Global domain and grid variables, updated when x_min changes
...
def print_stability_info():
    base_largeness = tau * (X * omega_0**2 + abs(b_ext) * nu_ext) + delta_v

    print(
        f"    dx = {delta_x:.12g}, dv = {delta_v:.12g}, "
        f"Nx = {Nx}, Nv = {Nv}"
    )
    print(
        f"    pre-run vertical size check without feedback: "
        f"V = {V:.6g}, rhs = {base_largeness:.6g}"
    )
    if V <= base_largeness:
        print(
            "    Warning: the vertical largeness condition already looks "
            "tight before the feedback term b_int*N(t)."
        )


def run_pde_once(K_value, x_min_value):
    ...
    update_kick_dependent_parameters(K_value)
    update_domain_parameters(x_min_value)
    init_xv(n)

    print(
        f"\n[PDE] x_min = {x_min:g}, K = {K:.1e}, "
...
def run_brian_ensemble(K_value, init_x_min):
    ...
    print(
        f"\n[Brian2] K = {K:.1e}, init lower x = {init_x_min:g}, "
        f"NE = {NE}, NI = {NI}, N = {N_BRIAN}"
    )
    ...
    try:
        ...
        brian_mean_x_runs_raw = np.empty((N_RUNS, Nt_user), dtype=float)
...
def plot_comparison_curves(outdir, stem, brian_available):
    ...
    ax1.set_title(
        rf"Population activity, $x_{{\min}}={x_min:g}$, $K={K:.0e}$"
    )
    ...
    ax2.set_title(
        rf"Mean voltage, $x_{{\min}}={x_min:g}$, $K={K:.0e}$"
    )
    ...
    ax3.set_title(
        rf"Final voltage density, $x_{{\min}}={x_min:g}$, $K={K:.0e}$"
    )
...
def plot_summary(records, outdir):
    ...
    xmins = sorted(set(float(get_record_value(row, 'x_min')) for row in records))
    k_values = sorted(set(float(get_record_value(row, 'K')) for row in records))

    def value_at(xmin_value, K_value, key):
        for row in records:
            if (
                float(get_record_value(row, 'x_min')) == xmin_value
                and float(get_record_value(row, 'K')) == K_value
            ):
                return float(get_record_value(row, key))
        return float('nan')

    for obs_name, key, ylabel in observables:
        # K variation: log-log plot, one curve for each x_min.
        fig, ax = plt.subplots(figsize=(7, 4))
        for xmin_value in xmins:
            yvals = np.array([value_at(xmin_value, K_value, key) for K_value in k_values])
            ax.loglog(
                k_values,
                yvals,
                marker='o',
                lw=1.0,
                label=rf"$x_{{\min}}={xmin_value:g}$",
            )
        ax.set_xlabel(r"$K$")
        set_horizontal_ylabel(ax, ylabel)
        ax.set_title(rf"Variation with $K$: {obs_name}")
        ...
        if SAVE_PDF:
            fig.savefig(
                os.path.join(outdir, f"summary_loglog_K_L2_{obs_name}.pdf"),
                bbox_inches='tight',
            )
        if SAVE_PNG:
            fig.savefig(
                os.path.join(outdir, f"summary_loglog_K_L2_{obs_name}.png"),
                dpi=DIAG_PLOT_DPI,
                bbox_inches='tight',
            )
        plt.close(fig)

        # x_min variation: ordinary plot, one curve for each K.
        fig, ax = plt.subplots(figsize=(7, 4))
        for K_value in k_values:
            yvals = np.array([value_at(xmin_value, K_value, key) for xmin_value in xmins])
            ax.plot(
                xmins,
                yvals,
                marker='o',
        ...
        set_horizontal_ylabel(ax, ylabel)
        ax.set_title(rf"Variation with $x_{{\min}}$: {obs_name}")
        ax.legend(frameon=False)
        fig.tight_layout()
        fig.subplots_adjust(left=0.30)
        ...
        if SAVE_PDF:
            fig.savefig(
                os.path.join(outdir, f"summary_xmin_L2_{obs_name}.pdf"),
                bbox_inches='tight',
            )
        if SAVE_PNG:
            fig.savefig(
                os.path.join(outdir, f"summary_xmin_L2_{obs_name}.png"),
                dpi=DIAG_PLOT_DPI,
                bbox_inches='tight',
            )
        plt.close(fig)
...
def print_global_parameters():
    update_kick_dependent_parameters(K_VALUES[0])
    ...
    print()
    print(f"T = {T:.6e} s")
    print(f"Nt = {Nt_user}, dt = {T / (Nt_user - 1):.6e} s")
    print(f"n = {n}, target h = {U0_volt / (n + 1):.12g}")
    print(f"XMIN_VALUES = {XMIN_VALUES}")
    print(f"K_VALUES    = {K_VALUES}")
    print(f"N_RUNS = {N_RUNS}, N_BRIAN = {N_BRIAN}")
    print(f"REUSE_BRIAN_ACROSS_XMIN = {REUSE_BRIAN_ACROSS_XMIN}")
    print()
...
def main():
    ...
    outdir = os.path.join(
        "results",
        "pde-vs-brian2-xmin-K-parallel-sweep",
        timestamp,
    )
    os.makedirs(outdir, exist_ok=True)

    records = []
    shared_init_x_min = min(XMIN_VALUES)
    t_all = time.time()
    ...
    for K_value in K_VALUES:
        ...
        print()
        print(f"K = {K:.1e}")
        print(f"b_ext = {b_ext:.6e}, b_int = {b_int:.6e}")
        print(f"a_0   = {a_0:.6e}, a_1   = {a_1:.6e}")

        brian_available_for_this_K = False
        if RUN_BRIAN and REUSE_BRIAN_ACROSS_XMIN:
            brian_available_for_this_K = run_brian_ensemble(
                K_value,
                shared_init_x_min,
            )

        for x_min_value in XMIN_VALUES:
            run_pde_once(K_value, x_min_value)

            brian_available = False
            if RUN_BRIAN:
                if REUSE_BRIAN_ACROSS_XMIN:
                    brian_available = brian_available_for_this_K
                else:
                    brian_available = run_brian_ensemble(K_value, x_min_value)

                if brian_available:
                    brian_available = build_brian_summary_on_current_grid()

            stem = (
                f"compare_xmin{fmt_xmin_for_filename(x_min_value)}_"
                f"K{fmt_float_for_filename(K_value)}_"
                f"n{n}_Nx{Nx}_Nv{Nv}_Nt{Nt_user}"
            )
    ...
    print(" - log-log K versus L2 distance")
    print(" - x_min versus L2 distance")
