# -*- coding: utf-8 -*-
"""
M-sweep version of the PDE/Brian2 comparison code.

The relative quantities correspond to \norm{PDE-Brian} / \norm{Brian}.

Outputs:
- for each M: comparison plots for N(t), X(t), rho(x,T);
- a CSV table of L2 distances against the Brian ensemble mean;
- log-log plots of M versus L2 distance.
...
# In the spirit of the structure of the paper we introduce K_FIXED
# as it is the value which intervenes in the diffusion approximation
X_MIN_FIXED = -21.0
J_FIXED = 1.0e-4
K_FIXED = 1.0e-5       # J_FIXED/(C_phys*tau_syn_phys), with C=1 and tau_syn=10

# Particle-number sweep values
...
# Brian parameters
N_RUNS = 10
NE = int(round(EXCITATORY_FRACTION * M_VALUES[0]))
NI = int(M_VALUES[0] - NE)
N_BRIAN = int(M_VALUES[0])
...
def run_pde_once():
    ...
    print(
        f"\n[PDE] n = {n}, dt = {delta_t:.6e}, "
        f"x_min = {x_min:g}, J = {J:.1e}, K = {kick_size:.1e}, "
...
def run_brian_ensemble(M_value):
    ...
    global NE, NI, N_BRIAN

    N_BRIAN = int(M_value)
    NE = int(round(EXCITATORY_FRACTION * N_BRIAN))
    NI = int(N_BRIAN - NE)

    update_kick_dependent_parameters(K_FIXED)

    print(
        f"\n[Brian2] M = {N_BRIAN}, NE = {NE}, NI = {NI}, "
        f"J = {J:.1e}, K = {kick_size:.1e}"
    )
...
def plot_summary(records, outdir):
    ...
    M_values = sorted(set(float(get_record_value(row, 'M')) for row in records))

    def value_at(M_value, key):
        for row in records:
            if float(get_record_value(row, 'M')) == M_value:
                return float(get_record_value(row, key))
        return float('nan')

    for obs_name, key, ylabel in observables:
        fig, ax = plt.subplots(figsize=(7, 4))
        yvals = np.array([value_at(M_value, key) for M_value in M_values])
        ax.loglog(M_values, yvals, marker='o', lw=1.0)
        ax.set_xlabel(r"$M$")
        set_horizontal_ylabel(ax, ylabel)
        ax.set_title(rf"Variation with $M$: {obs_name}")
        fig.tight_layout()
        fig.subplots_adjust(left=0.30)

        if SAVE_PDF:
            fig.savefig(
                os.path.join(outdir, f"summary_loglog_M_L2_{obs_name}.pdf"),
                bbox_inches='tight',
            )
        if SAVE_PNG:
            fig.savefig(
                os.path.join(outdir, f"summary_loglog_M_L2_{obs_name}.png"),
                dpi=DIAG_PLOT_DPI,
                bbox_inches='tight',
            )
        plt.close(fig)
...
def print_global_parameters():
    ...
    print("PHYSICAL PARAMETERS FOR M SWEEP")
    ...
    print(f"J fixed     = {J_FIXED:.6e}")
    print(f"K fixed     = {K_FIXED:.6e}")
    print(f"T           = {T:.6e} s")
    print(f"dt          = {T / (Nt_user - 1):.6e} s")
...
def main():
    ...
    for M_value in M_VALUES:
        ...
        if RUN_BRIAN:
            brian_available = run_brian_ensemble(M_value)
            if brian_available:
                brian_available = build_brian_summary_on_current_grid()

        stem = (
            f"compare_M{int(M_value)}_"
            f"J{fmt_float_for_filename(J_FIXED)}_"
    ...
    print("Saved per-M comparison graphs:")
    print(" - activity")
    print(" - mean voltage")
    print(" - final density")
    print("Saved summary graphs:")
    print(" - log-log M versus L2 distance")
