# -*- coding: utf-8 -*-
"""
The script performs an x_min sweep at fixed K in an inhibitory network.

Outputs:
- for each x_min value: comparison plots for N(t), X(t), rho(x,T);
- a CSV table of L2 distances against the Brian ensemble mean;
- summary plots of x_min versus L2 distance;
- ordinary plots of x_min versus L2 distance.

The relative plots correspond to divisions by the norm for the associated
Brian 2 observable
"""
...
x10 = 2.0
v10 = -15.0
sigma_init_x = 1.0 * U0_volt
sigma_init_v = 1.0 * U0_volt
...
def run_pde_one_case(x_min_value, kick_value):
...
def plot_summary(records, outdir):
    ...
    def value_at(xmin_value, key):
        for row in records:
            if float(get_record_value(row, 'x_min')) == xmin_value:
                return float(get_record_value(row, key))
        return float('nan')

    for obs_name, key, ylabel in observables:
        fig, ax = plt.subplots(figsize=(7, 4))
        yvals = np.array([value_at(xmin_value, key) for xmin_value in xmins])
        ax.plot(xmins, yvals, marker='o', lw=1.0)
        ax.set_xlabel(r"$x_{\min}$")
        set_horizontal_ylabel(ax, ylabel)
        format_yaxis_single_power(ax)
        ax.set_title(rf"Influence of $x_{{\min}}$: {obs_name}")
        fig.tight_layout()
...
def print_global_parameters():
    update_kick_dependent_parameters(KICK_VALUES[0])
    ...
    print(f"KICK_VALUES = {KICK_VALUES}")
...
def main():
    ...
    for kick_value in KICK_VALUES:
        update_kick_dependent_parameters(kick_value)

        print("\n")
        print(f"K = {kick_size:.1e}, reconstructed J_area = {J:.6e}")
        print(f"b_ext = {b_ext:.6e}, b_int = {b_int:.6e}")
        print(f"a_0   = {a_0:.6e}, a_1   = {a_1:.6e}")

        brian_available_for_this_J = False

        if RUN_BRIAN and REUSE_BRIAN_ACROSS_XMIN:
            brian_available_for_this_J = run_brian_ensemble(
                kick_value,
                shared_init_x_min,
            )

        for x_min_value in XMIN_VALUES:
            run_pde_one_case(x_min_value, kick_value)

            brian_available = False

            if RUN_BRIAN:
                if REUSE_BRIAN_ACROSS_XMIN:
                    brian_available = brian_available_for_this_J
                else:
                    brian_available = run_brian_ensemble(kick_value, x_min_value)

                if brian_available:
                    brian_available = build_brian_summary_on_current_grid()

            stem = (
                f"compare_xmin{fmt_xmin_for_filename(x_min)}_"
                f"kick{fmt_float_for_filename(kick_size)}_"
            ...
            # Save incrementally, so partial results are kept if the sweep stops.
