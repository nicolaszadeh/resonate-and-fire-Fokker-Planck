...
- the code then continues with the next (Nt,Nx,Nv),
- failed runs are stored as nan in the CSV
- the CSV gives the runtime for each of the (Nt,Nx,Nv)
"""
...
os.makedirs("results/sparse-solver", exist_ok=True)

scheme_name = "spsolver"
results_filename = f"results/sparse-solver/{scheme_name}_{timestamp}.csv"
...
input_seq_n_xv=[2,5,10,15]
input_seq_Nt=[500,1000,5000,10000]
...
try:
    ...
    for n,Nt in zip(input_seq_n_xv,input_seq_Nt):
        ...
        seq_dx.append(delta_x)
        seq_dv.append(delta_v)
        ...
        def build_A_sparse(N):
            """
            Builds the sparse version of the implicit diffusion operator in v.
            """
            ...
            main = np.ones(size_mat, dtype=np.float64)
            lower = np.zeros(size_mat - 1, dtype=np.float64)
            upper = np.zeros(size_mat - 1, dtype=np.float64)
            
            # Interior x-rows only: i = 1, ..., i_F - 1
            for i in range(1, i_F):
                ...
                # j = 1, ..., Nv interior in v
                js = row_start + np.arange(1, Nv + 1)
                main[js] = 1.0 + 2.0 * alpha
                lower[js - 1] = -alpha
                upper[js] = -alpha
            
                # j = Nv + 1 boundary in v
                m = row_start + Nv + 1
                main[m] = 1.0 + alpha
                lower[m - 1] = -alpha
        ...
        def solve_A_spsolve(D, N):
            """
            Solve A p^{n+1} = D using spsolve
            """
            size_mat = (Nx + 2) * (Nv + 2)
        
            A = build_A_sparse(N)
            rhs = D.reshape(size_mat)
        
            sol = spsolve(A, rhs)
        
            pnew = sol.reshape((Nx + 2, Nv + 2))
