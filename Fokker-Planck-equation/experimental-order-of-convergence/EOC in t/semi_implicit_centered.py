# -*- coding: utf-8 -*-
"""
Fast semi-implicit centered version:
- diffusion in v implicit at time n+1,
- transport explicit at time n,
- centered finite differences in the interior,
- one-sided/upwind treatment on x-boundaries,
...
os.makedirs("Results/Semi_implicit_centered", exist_ok=True)

scheme_name = "Current_fast_semi_implicit_centered_time_EOC"
results_filename = f"Results/Semi_implicit_centered/{scheme_name}_{timestamp}.csv"
...
try:
    ...
    def apply_B_2d(p, N):
        """
        Explicit centered transport at time n:
        - centered in x in the interior,
        - one-sided/upwind on x-boundaries,
        - centered in v in the interior,
        - Robin-type transport treatment at j=0 and j=Nv+1,
        - NO j_c correction.
        """
        out = p.copy()
        ...
        # left boundary i = 0 : one-sided only if v < 0
        if j_0 > 0:
        ...
        # right boundary i = i_F : one-sided only if v > 0
        if j_0 + 1 < Nv + 2:
            coeff_right = (-v[j_pos_x] * dt_over_dx)[None, :]
            out[i_F:i_F+1, j_pos_x] += coeff_right * (
                p[i_F:i_F+1, j_pos_x] - p[i_F-1:i_F, j_pos_x]
            )
