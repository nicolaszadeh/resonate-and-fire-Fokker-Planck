...
os.makedirs("results/explicit-upwind", exist_ok=True)

scheme_name = f"explicit_upwind_EOC_xv_{restriction_mode}"
results_filename = f"results/explicit-upwind/{scheme_name}_{timestamp}.csv"
...
try:
    ...
    def init_xv(n):
        ...
        print("The mass of the concentrated maxwellian is equal to:",
              delta_x * delta_v * np.sum(rho, dtype=np.float64))
    ...
    def apply_B_2d(p, N):
        ...
        # Explicit diffusion in v, only on interior x-rows
        if i_F > 1:
            ...
            # top boundary j = Nv+1
            out[1:i_F, Nv+1] += alpha * (
                p[1:i_F, Nv] - p[1:i_F, Nv+1]
            )
    ...
    for n in seq_Nxv:
        ...
        try:
            ...
            print(
                ...
                f"CFL_transport={bool(CFL_transport)}, "
    ...
    for idx, n in enumerate(seq_Nxv):
        ...
        V_large_enough_i, CFL_transport_i, positivity_i, mass_cons_i = seq_flags[idx]
