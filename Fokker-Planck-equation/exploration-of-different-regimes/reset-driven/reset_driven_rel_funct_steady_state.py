...
steady_state_dir = os.path.join(results_dir, "steady-state")
...
def make_density_figure(frame2d, title, norm, colorbar_label=r"$f$"):
    ...
    ax.seax.tick_params(axis="y", which="both", left=False, right=False, labelleft=False)t_title(title)
...
def save_steady_state_3d_views(f_inf, filename_stem):
    ...
    outdir_3d = os.path.join(steady_state_dir, "steady-state-3d-views")
