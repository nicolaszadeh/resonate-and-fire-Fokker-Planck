# -*- coding: utf-8 -*-
"""
Stationary states and relative functionals for the feedback-driven-oscillations regime
...
steady_state_dir = os.path.join(results_dir, "long-time")
...
def set_separated_uR_uF_xticklabels_2d(ax, y_offset=-0.055):
    """
    Put ticks at u_R and u_F, but place the labels manually, they were 
    overlapping before.
    """
...
def set_separated_uR_uF_xticklabels_3d(ax, left_tick):
    """
    Same idea for 3D plots: keep the ticks, but align u_R and u_F
    away from each other.
    """
    ax.set_xticks([left_tick, u_R, u_F])
...
def make_density_figure(frame2d, title, norm, colorbar_label=r"$f$"):
    ...
    set_separated_uR_uF_xticklabels_2d(ax)
    ax.yaxis.set_major_formatter(tex_relevant_tick_formatter())
...
def save_steady_state_3d_plot(f_inf, N_inf, filename_base):
    ...
    x_min_plot = -6
    x_max_plot = 9
    x_mask = (x >= x_min_plot) & (x <= x_max_plot)
    ...
    x_plot=x[x_mask]    
    v_plot = v[v_mask]
    Z3 = maybe_smooth(f_inf)[np.ix_(x_mask, v_mask)]

    X3, V3 = np.meshgrid(x_plot, v_plot, indexing="ij")
    ...
    set_separated_uR_uF_xticklabels_3d(ax, left_tick=x_min_plot)
...
def save_steady_state_3d_views(f_inf, filename_stem):
    ...
    X3, V3 = np.meshgrid(x_sub, v_sub, indexing="ij")
    zmax = np.max(f_plot[:,:])
    ...
    outdir_3d = os.path.join(steady_state_dir, "long-time-3d-views")
    ...
    for name, elev, azim in views:
        ...
        polish_3d_axes(ax, elev=elev, azim=azim)

        set_separated_uR_uF_xticklabels_3d(ax, left_tick=xmin)
        
        yticks = np.array([vmin, 0, vmax], dtype=float)
        ...
        pdf_name = os.path.join(
            outdir_3d,
            f"{filename_stem}_{local_stem}_long_time_3D_no_colorbar_{name}.pdf"
        )
...
def rerun_after_reference(f_inf, N_inf):
    ...
    steady_base = os.path.join(steady_state_dir, file_stem + "_long_time")
