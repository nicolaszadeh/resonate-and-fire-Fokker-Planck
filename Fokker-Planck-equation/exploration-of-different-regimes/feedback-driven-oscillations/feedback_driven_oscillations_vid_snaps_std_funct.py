...
in the feedback-driven-oscillations regime
...
def tex_cbar_num(x, pos=None):
    if abs(x) < 5e-15:
        x = 0.0
    return rf"${x:.3f}$"

def tex_cbar_tick_formatter():
    return mticker.FuncFormatter(tex_cbar_num)

def apply_tex_colorbar_ticks(cbar):
    cbar.ax.yaxis.set_major_formatter(tex_cbar_tick_formatter())
    cbar.update_ticks()
...
def save_snapshot_grid(frames, times_snap, outdir, stem, norm):
    ...
    for k, (ax, frame, tval) in enumerate(zip(axes.flat, frames, times_snap)):
        ...
        if row == 2:
            ...
            # Keep the tick marks at u_R and u_F, but remove the automatic labels.
            ax.set_xticks([u_R, u_F])
            ax.set_xticklabels(["", ""])

            # Manually place the labels so they do not overlap.
