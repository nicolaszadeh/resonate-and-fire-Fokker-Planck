...
def save_steady_state_3d_plot(f_inf, N_inf, filename_base):
    ...
    fig.text(0.15, 0.35, r"$u_{\rm R}$")
    fig.text(0.17, 0.32, r"$u_{\rm F}$")
    ...
    try:
        ax.view_init(elev=elev, azim=azim, roll=2)
    except TypeError:
    ...
    cbar = fig.colorbar(surf, ax=ax, shrink=0.4, pad=0.2)
    apply_tex_colorbar_ticks(cbar)
    ...
    fig.subplots_adjust(left=0.06, right=0.72, bottom=0.10, top=0.95)
